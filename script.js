...
function renderMessagesList() {
    ...
    listEl.innerHTML = entries.map(([tag, chat]) => {
        ...
        let preview = 'Démarrer la conversation';
        if (last) {
            if (last.kind === 'image') preview = 'Photo';
            else if (last.kind === 'video') preview = 'Vidéo';
            else if (last.kind === 'audio') preview = 'Audio';
            else if (last.kind === 'file') preview = last.name || 'Fichier';
...
const allExtraCategories = [
    ...
    { name: "Vêtements bébé", icon: "baby" }, { name: "Équipement bébé", icon: "stroller" }, { name: "Jeux & Jouets", icon: "toy-brick" },
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const lastSection = localStorage.getItem('winjayLastSection') || 'home-section';
...
themeToggle.addEventListener('click', () => {
    ...
    themeToggle.querySelector('span').textContent = isDarkMode ? 'Mode Jour' : 'Mode Nuit';
    applyWinjayLogoTheme();
    lucide.createIcons();
});
...
document.getElementById('addListingForm').addEventListener('submit', (e) => {
    ...
    showToast('Annonce publiée avec succès !', 'check-circle');
    e.target.reset();
...
document.getElementById('editProfileForm').addEventListener('submit', (e) => {
    ...
    if (!tagValue) {
        showToast('Le nom d\'utilisateur est requis', 'alert-circle');
        return;
    }

    if (/\s/.test(tagValue)) {
        showToast('Le pseudo ne peut pas contenir d\'espaces', 'alert-circle');
        return;
    }

    if (!/^[a-z0-9_.]+$/.test(tagValue)) {
        showToast('Pseudo invalide (lettres, chiffres, points, underscores uniquement)', 'alert-circle');
        return;
    }
    ...
    if (allTags.includes(tagValue) && tagValue !== userProfile.tag) {
        showToast('Ce pseudo est déjà pris', 'user-x');
        return;
    }
    ...
    showToast('Profil mis à jour !', 'check-circle');
});

document.getElementById('contactForm').addEventListener('submit', (e) => {
    e.preventDefault();
    showToast('Message envoyé avec succès !', 'send');
...
document.getElementById('loginForm').addEventListener('submit', (e) => {
    ...
    if (!client) {
        showToast('Supabase non configuré', 'alert-circle');
        return;
    }
    ...
    client.auth.signInWithPassword({ email, password }).then(({ error }) => {
        if (error) {
            showToast(error.message || 'Erreur de connexion', 'alert-circle');
            return;
        }
        showToast('Connexion réussie !', 'check-circle');
...
document.getElementById('registerForm').addEventListener('submit', (e) => {
    ...
    client.auth.signUp({
        ...
        if (error) {
            showToast(error.message || "Erreur lors de l'inscription", 'alert-circle');
            return;
        }
        if (!data?.session) {
            showToast("Vérifiez votre email pour confirmer l'inscription", 'mail');
        ...
        showToast('Compte créé avec succès !', 'check-circle');
        closeModal('registerModal');
        e.target.reset();
        updateProfileUI();
...
function formatUsernameInput(input) {
    ...
    if (/\s/.test(value)) {
        statusEl.textContent = '❌ Espaces non autorisés';
        statusEl.style.color = '#dc3545';
        return;
    }

    if (!/^[a-z0-9_.]+$/.test(value)) {
        statusEl.textContent = '❌ Caractères non autorisés';
        statusEl.style.color = '#dc3545';
        return;
    }
    ...
    if (allTags.includes(currentTag) && currentTag !== userProfile.tag) {
        ...
        statusEl.textContent = '✓ Pseudo disponible';
...
function loginWithGoogle() {
    ...
    showToast('Connexion Google en cours...', 'log-in');
    client.auth.signInWithOAuth({
        ...
        if (error) showToast(error.message || 'Erreur Google', 'alert-circle');
    });
}
...
function openForgotPassword() {
    ...
    const email = document.getElementById('loginEmail')?.value?.trim();
    if (!email) {
        showToast('Entrez votre email', 'alert-circle');
        return;
    }
    client.auth.resetPasswordForEmail(email, { redirectTo: window.location.origin }).then(({ error }) => {
        ...
        closeModal('loginModal');
        showToast('Un email de réinitialisation a été envoyé', 'mail');
    });
}
...
function handleLogout() {
    showConfirmModal(
        ...
        async () => {
            ...
            showToast('Déconnexion réussie !', 'log-out');
            showSection('home-section');
        }
    );
}
